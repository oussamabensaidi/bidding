...
<div class="grid grid-cols-3 gap-4 mb-4">
    ...
    <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 m-4 relative">
        ...
        <span style="position: absolute;top: 0;right: 0;">
            <form action="{{route('items.delete-image')}}" method="POST">
                @csrf
                @method('DELETE')
                 <input type="hidden" name="item_id" value="{{ $item->id }}">{{-- we could use the route parameter instead but the hidden input are better for security --}}
